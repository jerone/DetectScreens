...
namespace DetectScreens.Store
{
	...
	public partial class FrmDetectScreens : Form
	{
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			if (!HelperFramework.Reflection.Instance.IsSingleInstance(Text))
			{
				Close();
			}
		}
		...
		protected override void WndProc(ref Message m)
		{
			...
			if (DRAWGLASS)
			{
				switch ((uint)m.Msg)
				{
					...
					case Win32.WM.THEMECHANGED:
					case Win32.WM.DWMCOMPOSITIONCHANGED:
						{
							_glassMargins.Top = ClientSize.Height - panel1.Height;
							DwmGlass.DwmExtendFrameIntoClientArea(Handle, _glassMargins);
							break;
						}
		...
		private void BtnPreviewClick(Object sender, EventArgs e)
		{
			if (_showIdentifys)
			{
				...
				foreach (Display display in displays)
				{
					FrmIdentify preview = new FrmIdentify
						{
							Nr = display.Identifier.ToString(CultureInfo.InvariantCulture)
						};
					Window.SetWindowPos(preview.Handle, IntPtr.Zero, 0, 0, 0, 0, Win32.SWP.NOSIZE | Win32.SWP.NOMOVE | Win32.SWP.NOACTIVATE);  // Allow preview rectangles to draw over the taskbar (must be declared before setting the bounds!);
					...
					preview.KeyUp += delegate(Object sender2, KeyEventArgs e2)
					{
						...
						{
							_previews.ForEach(__preview2 => __preview2.Close());
			...
			{
				_previews.ForEach(__preview => __preview.Close());
			}
			_showIdentifys = !_showIdentifys;
		}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void BtnResolutionClick(Object sender, EventArgs e)
		{
			Window.WinExec("control.exe desk.cpl,Settings,@Settings", Win32.SW.SHOW);
		}
