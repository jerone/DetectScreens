...
namespace DetectScreens.Store
{
	...
	public partial class FrmDetectScreens : Form
	{
		...
		public FrmDetectScreens()
		{
			...
			lblVersionInfo.Text = String.Format(lblVersionInfo.Text, this.GetType().Assembly.GetName().Version.ToString());
		}
			...
			if (DRAWGLASS)
			{
				switch ((uint)m.Msg)
				{
					...
					case Win32.WM.WINDOWPOSCHANGED:
						{
							_glassMargins.Top = this.ClientSize.Height - panel1.Height;
							Win32.DwmExtendFrameIntoClientArea(this.Handle, _glassMargins);
							break;
						}
		...
		private void BtnPreviewClick(Object sender, EventArgs e)
		{
			if (_showIdentifys)
			{
				...
				foreach (Display display in displays)
				{
					...
					preview.KeyUp += delegate(Object _sender, KeyEventArgs _e)
					{
						if (_e.KeyCode == Keys.Escape)
						{
							_previews.ForEach(_preview => _preview.Close());
