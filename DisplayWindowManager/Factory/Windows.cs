...
namespace DisplayWindowManager.Factory
{
	...
	public class Window : IDisposable, IWin32Window
	{
		...
		public Window(IntPtr handle)
		{
			...
			HelperFramework.PInvoke.Window.GetWindowText(handle, sb, sb.Capacity);
			Title = sb.ToString();
		...
		public Boolean Unregister()
		{
			...
			if (Thumb != IntPtr.Zero)
			{
				try
				{
					if (DwmThumbnail.DwmUnregisterThumbnail(Thumb) != 0)
					{
						success = false;
					}
				}
	...
	public class Desktop : Window
	{
		...
		/// </summary>
		public Desktop()
			: base(HelperFramework.PInvoke.Window.GetShellWindow())
		{
			Maximized = true;
		}
	}
	...
	public class Taskbar : Window
	{
		...
		/// </summary>
		public Taskbar()
			: base(HelperFramework.PInvoke.Window.FindWindow("Shell_TrayWnd", ""))
		{
	...
	public class Windows : List<Window>, IDisposable
	{
		...
		{
			const UInt64 WindowSearchMatch = (Win32.WS.TARGETWINDOW /* | Win32.WS.THICKFRAME*/);  // 'normal' window;

			HelperFramework.PInvoke.Window.EnumWindows(delegate(IntPtr hwnd, int lParam)
				{
					if ((HelperFramework.PInvoke.Window.GetWindowLongA(hwnd, Win32.GWL.STYLE) & WindowSearchMatch) == WindowSearchMatch)
					{
...
@"Window {0}:
	...
	Maximized: {4}",
		...
		{
			if ((HelperFramework.PInvoke.Window.GetWindowLongA(hwnd, Win32.GWL.STYLE) & Win32.WS.TARGETWINDOW) == Win32.WS.TARGETWINDOW)  // Only add 'normal' windows;
			{
				Window window = new Window(hwnd);
				Add(window);
			}
		...
		{
			Window window2Move = Find(__window => __window.Handle == hwnd);
			if (window2Move != null)
			{
				Remove(window2Move);

				// Get index of last window in the list. We don't want the taskbar;
				Int32 index = FindLastIndex(__window => __window.GetType() == typeof(Window)) + 1;

				Insert(index, window2Move);

				// Set current window and all next ones (e.g. Taskbar) to force refresh;
				GetRange(index, Count - index).ForEach(__window => __window.ForceRefresh = true);
			}
		}
		...
		{
			Window window2Remove = Find(__window => __window.Handle == hwnd);
		...
		public Boolean Unregister()
		{
			return this.All(__window => __window.Unregister());
		}
		...
		private void Dispose(Boolean disposing)
		{
			if (!_disposed)
			{
				if (disposing)
				{
					ForEach(__window => __window.Dispose());
				}
